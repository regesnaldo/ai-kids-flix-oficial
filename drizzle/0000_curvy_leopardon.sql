CREATE TABLE `agent_combinations` (
	...
	`descricao` text,
	`requisitos_desbloqueio` json DEFAULT ('{}'),
...
CREATE TABLE `agent_memories` (
	...
	`memory_type` enum('emotional','factual','preference','narrative') NOT NULL DEFAULT 'factual',
...
CREATE TABLE `profiles` (
	...
	`avatar_shape` varchar(20) DEFAULT 'humanoid',
	`avatar_color` varchar(7) DEFAULT '#3B82F6',
	`aura_color` varchar(7) DEFAULT '#60A5FA',
...
--> statement-breakpoint
ALTER TABLE `agent_memories` ADD CONSTRAINT `agent_memories_user_id_users_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `idx_am_user_agent` ON `agent_memories` (`user_id`,`agent_id`);--> statement-breakpoint
CREATE INDEX `idx_am_user_emotional` ON `agent_memories` (`user_id`,`memory_type`);--> statement-breakpoint
CREATE INDEX `idx_am_expires` ON `agent_memories` (`expires_at`);--> statement-breakpoint
CREATE INDEX `idx_am_user_access` ON `agent_memories` (`user_id`,`access_count`);--> statement-breakpoint
