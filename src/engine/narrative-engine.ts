 ...
 * LangChain — apenas raw fetch para OpenAI. Renomeado para clareza.
 */

import { getConflictForAgents, type AgentId } from './agent-conflicts';
...
export async function analyzeNarrative(
  ...
  if (totResult) {
    ...
    const conflictTriggered = !!getConflictForAgents(currentAgent, nextAgent);
