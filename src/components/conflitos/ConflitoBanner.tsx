...
export default function ConflitoBanner() {
  ...
  const conflitoAtual = conflito;

  function handleEscolha(posicao: "AGENTE_1" | "AGENTE_2" | "NEXUS") {
    responderConflito({
      conflitoId: conflitoAtual.id,
  ...
  return (
    <section className="rounded-xl border border-white/10 bg-black/40 p-4">
      ...
      <h3 className="mt-1 text-lg font-bold text-white">{conflitoAtual.titulo}</h3>
      <p className="mt-2 text-sm text-white/80">{conflitoAtual.descricao}</p>
