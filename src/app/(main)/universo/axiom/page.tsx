...
const AxiomScene = dynamic(
  () => import('@/components/scenes/AxiomScene').then((m) => m.AxiomScene),
...
const AGENT = {
  ...
  role: 'O Cientista',
  description: 'Demonstrar como modelos de IA aprendem com dados e o que acontece quando os dados são ruins.',
  approach: 'Rigoroso, preciso, orientado a dados. "Sem dados, é apenas uma opinião."',
  color: '#0EA5E9',
  values: ['dados', 'precisão', 'hologramas', 'método'],
}

export default function AxiomUniversePage() {
  return (
    <main className="relative w-full h-screen overflow-hidden" style={{ background: '#0a0a1a' }}>
      <div className="absolute inset-0 z-0"><Canvas><AxiomScene /></Canvas></div>
