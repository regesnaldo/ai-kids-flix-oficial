...
import { StratosScene } from "@/components/universo/StratosScene";

const STRATOS_VOICE_ID = process.env.NEXT_PUBLIC_STRATOS_VOICE_ID || "g6xIsTj2HwM6VR4iXFCw";
...
export default function StratosUniversePage() {
  ...
  return (
    <main className="relative h-screen w-full overflow-hidden bg-[#01050c]">
      {!introConcluida && (
        <NexusCinematicIntro
          onComplete={() => setIntroConcluida(true)}
          onSkip={() => setIntroConcluida(true)}
          voiceId={STRATOS_VOICE_ID}
          audioEnabled={audioEnabled}
        />
      )}

      <div className="absolute inset-0 z-0">
        <Canvas camera={{ position: [10, 3, 10], fov: 58 }} dpr={[1, 2]}>
          <Suspense fallback={null}>
            <StratosScene />
          </Suspense>
        </Canvas>
        <Loader containerStyles={{ background: "#01050c" }} innerStyles={{ background: "#06b6d4" }} />
      </div>
      ...
      {introConcluida && (
        <>
          ...
          >
            <div>
              <p className="font-mono text-sm font-black tracking-widest text-cyan-300">STRATOS</p>
              <p className="text-xs text-cyan-100/70">Torre estratégica de longo prazo</p>
            </div>
          ...
          <div className="absolute inset-0 z-10 pointer-events-none">
            <NexusDialogLive
              endpoint="/api/universo/stratos"
              agentLabel="STRATOS"
              initialAssistantMessage="STRATOS: Liste três caminhos possíveis. Vamos montar uma árvore de decisão com impacto em 30, 90 e 365 dias."
              onSpeak={handleSpeak}
